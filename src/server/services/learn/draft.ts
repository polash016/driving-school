...
import { draftDocumentInputSchema, draftResultSchema, type DraftResult } from "@/server/contracts/learn";
import { legalCitationSchema } from "@/server/contracts/models";
import { checkTranslation } from "@/server/services/i18n/validation";
import { lookupChunksForCitations, type CitationLookup, type CitationRef } from "@/server/services/kb/citations";
...
export interface DraftDeps {
  ...
  ) => Promise<Array<{ chunkId: string; sourceCode: string; ref: string; text: string }>>;
  resolve: (db: PrismaClient, citations: CitationRef[]) => Promise<CitationLookup>;
}
  ...
  retrieve: async (db, input) => {
    if (input.sectionRef && input.sourceCodes.length > 0) {
      ...
      const { resolved } = await lookupChunksForCitations(
        db,
        input.sourceCodes.map((sourceCode) => ({ sourceCode, ref: input.sectionRef! })),
        { perCitation: 6 },
      );
      if (resolved.length > 0) {
        return resolved.map((c) => ({ chunkId: c.kbChunkId, sourceCode: c.sourceCode, ref: c.chunkRef, text: c.text }));
      }
    }
    const result = await search(db, {
      query: input.sectionRef ? `${input.query} ${input.sectionRef}` : input.query,
      limit: 12,
      ...(input.sourceCodes.length > 0 ? { sourceCodes: input.sourceCodes } : {}),
    });
    return result.hits.map((hit) => ({ chunkId: hit.chunkId, sourceCode: hit.sourceCode, ref: hit.ref, text: hit.text }));
  },
  resolve: (db, citations) => lookupChunksForCitations(db, citations, { perCitation: 1 }),
};
...
export function structureMismatch(en: string, nb: string): string | null {
  ...
  if (a.h2 !== b.h2) return `English has ${a.h2} H2 sections, Norwegian ${b.h2}.`;
  if (a.h3 !== b.h3) return `English has ${a.h3} H3 headings, Norwegian ${b.h3}.`;
...
export function stripForbidden(markdown: string): { markdown: string; removed: string[] } {
...
export async function draftDocument(
  ...
  if (!topic) throw new ValidationError({ topicId: input.topicId }, "admin.learn.errors.topicMissing");
  ...
  const [book, siblings] = await Promise.all([
    input.bookId
      ? db.learnBook.findFirst({ where: { id: input.bookId, deletedAt: null }, select: { title: true } })
  ...
  const siblingTitles = siblings.map((row) => `- ${pickBilingualText(row.title, "en")}`).join("\n");
  ...
  if (hits.length < MIN_EXCERPTS) {
    throw new ValidationError({ topicId: topic.id, found: hits.length }, "admin.learn.errors.aiNoMaterial");
  }
  ...
  const kbExcerpts = kept.map((hit) => `[${hit.sourceCode} ${hit.ref}]\n${hit.text}`).join("\n\n");
  ...
  const resolvedRefs = new Map(resolved.map((c) => [`${c.sourceCode}|${c.ref}`, c]));
  if (resolvedRefs.size === 0) {
    throw new ValidationError({ citations }, "admin.learn.errors.aiNoCitations");
  }
  // Every § the body mentions should be in the citation list, or the sources footer lies by omission.
  const mentioned = new Set((en.markdown.match(/§\s*\d+(?:[.\-]\d+)*/gu) ?? []).map((s) => s.replace(/\s+/g, " ")));
  const listed = new Set(citations.map((c) => c.ref.replace(/\s+/g, " ")));
  if ([...mentioned].some((ref) => ![...listed].some((l) => l.startsWith(ref) || ref.startsWith(l)))) {
    warnings.push("uncitedReference");
  }
  const wordsEn = countWords(en.markdown, "en");
  if (wordsEn < input.targetWords * 0.5 || wordsEn > input.targetWords * 1.6) warnings.push("length");

  logger.info(
    { actorId: actor.id, topicId: topic.id, excerpts: kept.length, promptTokens: response.promptTokens, completionTokens: response.completionTokens, warnings },
  ...
  return draftResultSchema.parse({
    ...
    citations: citations.filter((c) => resolvedRefs.has(`${c.sourceCode}|${c.ref}`)),
...
function driftBetween(en: string, nb: string): string | null {
  ...
  const blocking = check.issues.filter((issue) => issue.blocking && issue.code !== "UNTRANSLATED");
  if (blocking.length === 0) return null;
  return blocking.map((issue) => `${issue.code}${issue.detail ? ` (${issue.detail})` : ""}`).join("; ");
}
