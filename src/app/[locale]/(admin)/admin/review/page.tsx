...
import { ReviewQueue, type ReviewItem } from "@/components/admin/questions/review-queue";
...
export default async function ReviewPage({
  ...
  const items: ReviewItem[] = rows.map((row) => {
    ...
    const side = content[contentSideFor(locale)] ?? content.en ?? content.nb ?? {};
    return {
      id: row.id,
      approvalsRecorded: row._count.approvals,
      approvalsRequired: requiredApprovals(row.createdBy, policy.aiApprovalsRequired),
      ...
      citations: (row.legalCitations ?? []) as { sourceCode: string; ref: string }[],
    };
  });
