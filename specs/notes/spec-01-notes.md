...
- Grep evidence: `grep -rn "TeoriPro|teoripro|999999999" src/` → only two CSS _comments_ in globals.css; `grep -rnE '#hex|oklch\(' src/ --include=*.ts*` → none (raw colors exist only in `config/theme.css` and `public/logo*.svg`, both school-swappable).
