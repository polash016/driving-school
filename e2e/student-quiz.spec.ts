...
test("practice: start, answer, hand in, and never receive the answer key early", async ({
  ...
  // options and moves what is under the pointer. (Worth noting as a real defect, not just a test
  // timing problem — spec-08 asks for CLS ≈ 0 on answer reveal.)
  ...
  phase = "post-submit";
  await page.getByRole("button", { name: /Hand in/ }).first().click();
...
test("setup screen says up front whether a test counts towards the guarantee", async ({
  ...
  await expect(page.getByText("This test will count towards the pass guarantee.")).toBeVisible();
...
test("an answer cannot be changed once it is given, and the test resumes where it was left", async ({
  ...
  await page.getByRole("navigation", { name: "Jump to question" }).getByRole("button").first().click();
  await expect(page.getByText("Answer recorded — it cannot be changed.")).toBeVisible();
  await expect(page.getByRole("radio").first()).toBeDisabled();
